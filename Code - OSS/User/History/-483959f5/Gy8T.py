...
def generate_sparse_matrix(size: int, num_nonzero: int) -> Tuple[List[float], List[int], List[int], Tuple[int, int]]:
    """Генерирует случайную разреженную матрицу размером size x size с num_nonzero ненулевыми элементами."""
    ...
    # Создаем разреженную матрицу в COO формате
    sparse_matrix = coo_matrix((values, (row_indices, col_indices)), shape=(size, size))

    # Преобразуем в CSR формат
    return sparse_matrix.data.tolist(), sparse_matrix.col.tolist(), sparse_matrix.indptr.tolist(), (size, size)
...
def main():
    ...
    # Генерация разреженной матрицы 10^6 x 10^6 с 10^6 ненулевыми элементами
    size = 10**6  # Размер матрицы 1e6 x 1e6
    num_nonzero = 10**6  # Количество ненулевых элементов
    values, indices, indptr, shape = generate_sparse_matrix(size, num_nonzero)
    matrix_keeper1.values = values
    matrix_keeper1.indices = indices
    matrix_keeper1.indptr = indptr
    matrix_keeper1.shape = shape
