...
class MatrixKeeper:
    ...
    def inputMatrix(self):
        # Генерация разреженной матрицы размера 10^4 x 10^4
        size = 10**4
        num_nonzero = 13218  # Увеличиваем количество ненулевых элементов
        sparse_matrix = generate_sparse_matrix(size, num_nonzero)
...
def generate_sparse_matrix(size: int, num_nonzero: int) -> sp.csr_matrix:
    """Генерирует разреженную матрицу размера size x size с num_nonzero ненулевыми элементами."""
    
    # Максимальное количество ненулевых элементов, которое возможно в матрице
    max_nonzero = size * size
    
    # Проверяем, что количество ненулевых элементов не превышает максимально возможное
    num_nonzero = min(num_nonzero, max_nonzero)
    
    # Генерация случайных индексов для строк и столбцов
    row_indices = np.random.choice(size, num_nonzero, replace=True)
    col_indices = np.random.choice(size, num_nonzero, replace=True)
    ...
    sparse_matrix = sp.csr_matrix((values, (row_indices, col_indices)), shape=(size, size))
...
def main():
    ...
    print(f"Количество ненулевых элементов: {len(matrix_keeper.values)}")
