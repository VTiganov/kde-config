...
void dijkstra(const std::vector<Node>& nodes, int start, int goal) {
    ...
    // Debugging: Print the contents of the came_from map
    std::cout << "came_from map contents:\n";
    for (const auto& pair : came_from) {
        std::cout << pair.first << " came from " << pair.second << "\n";
    }

    // Debugging: Print the contents of the distances map
    std::cout << "distances map contents:\n";
